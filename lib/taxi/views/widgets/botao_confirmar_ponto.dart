...
class _BotaoConfirmarState extends State<BotaoConfirmar> {
  ...
  Widget _botaoConfirmar(MapaController mapa, ThemeProvider tema) {
    ...
    final bool isTelaGrande = larguraTela > 780;

    if (!isTelaGrande) {
      // Comportamento original (NÃO muda nada no mobile)
      return SizedBox(
    ...
    // Responsivo em telas maiores que 780px
    return Align(
      alignment: Alignment.center,
      child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 400), // limite no desktop
        child: SizedBox(
          height: AppConstants.buttonHeight,
          child: ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: tema.primaryColor),
  ...
  Widget _botoesCadastrarExcluir(MapaController mapa, ThemeProvider tema) {
    ...
    if (!isTelaGrande) {
      // Layout padrão para telas menores que 780px (horizontal)
      return Row(
        children: [
    ...
    // Layout responsivo para telas maiores que 780px (vertical)
    return Align(
      alignment: Alignment.center,
      child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 400), // limite de largura
        child: Column(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            ...
            OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(
                backgroundColor: Colors.redAccent,
                side: BorderSide.none,
                padding: const EdgeInsets.symmetric(vertical: 16, horizontal: 24), // Aumentado padding
              ),
              onPressed: () => mapa.desfazerUltimoPonto(), // volta o ícone central
            ),
